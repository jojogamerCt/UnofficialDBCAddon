...
extends EntityDBCEvil {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(400000.0);
        this.getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(50000.0);
    }
    ...
    public void onLivingUpdate() {
        ...
        if (this.worldObj.isRemote && JGConfigClientSettings.CLIENT_DA8) {
            for (int k = 0; k < JGConfigClientSettings.get_da1(); ++k) {
                for (int i = 0; i < 5; ++i) {
                    EntityDBSBrolyLegendary pl = this;
                    float a = 0.5f;
                    float h1 = 1.0f;
                    float red = 183.0f;
                    float green = 205.0f;

...
import JinRyuu.DragonBC.common.Worlds.x1MapGenCavesOW;
import JinRyuu.DragonBC.common.Worlds.x1MapGenRavineOW;
...
implements IChunkProvider {
    ...
    private MapGenBase caveGenerator = new x1MapGenCavesOW();
    private MapGenScatteredFeature scatteredFeatureGenerator = new MapGenScatteredFeature();
    private MapGenBase ravineGenerator = new x1MapGenRavineOW();
    ...
    public x1ChunkProviderOW(World world, long seed, boolean mapFeaturesEnabled) {
        ...
        this.mobSpawnerNoise = new NoiseGeneratorOctaves(this.rand, 4);
    ...
    public void genBiomeModdedTerrain(BiomeGenBaseDBC bgb, World world, Random random, Block[] replacableBlock, byte[] aByte, int x, int y, double z) {
        Block block = BlocksDBC.BlockYellowCloud;
        byte b0 = (byte)(bgb.field_150604_aj & 0xFF);
        Block block1 = BlocksDBC.BlockYellowCloud;
        int k = -1;
        ...
        for (int l1 = 255; l1 >= 0; --l1) {
            ...
            if (l1 == 1) {
                replacableBlock[i2] = Blocks.lava;
            } else if (l1 <= 0 + random.nextInt(5)) {
                    ...
                    if (block2 == Blocks.stone) {
                        if (k == -1) {
                            if (l <= 0) {
                                ...
                                block = BlocksDBC.BlockYellowCloud;
                                b0 = (byte)(bgb.field_150604_aj & 0xFF);
                                block1 = BlocksDBC.BlockYellowCloud;
                            }
                            if (l1 < 40 && (block == null || block.getMaterial() == Material.air)) {
                                block = Blocks.water;
                                b0 = 0;
                            }
                            k = l;
                        ...
                        if (height == 0) {
                            height = l1;
                        } else if (height != 0 && height - hch < l1) {
                            replacableBlock[i2] = o85 && l1 >= 80 && 80 + (height - 80) / 10 == l1 ? BlocksDBC.BlockYellowCloud : (height - hch + 1 >= l1 && height - hch + 1 <= l1 ? bgb.fillerBlock : Blocks.air);
                        }
                    }
                } else {
                    k = -1;
            ...
            Block var38 = replacableBlock[i2];
            if (l1 >= 200 || var38 != Blocks.water) continue;
            replacableBlock[i2] = Blocks.air;
        }
    }
    ...
    public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType, int par2, int par3, int par4) {
        ...
        return par1EnumCreatureType == EnumCreatureType.monster && this.scatteredFeatureGenerator.func_143030_a(par2, par3, par4) ? this.scatteredFeatureGenerator.getScatteredFeatureSpawnList() : (r == 0 ? biomegenbase.getSpawnableList(par1EnumCreatureType) : new ArrayList());
    }

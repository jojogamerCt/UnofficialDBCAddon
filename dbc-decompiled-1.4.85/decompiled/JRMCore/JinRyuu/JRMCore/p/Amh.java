...
implements IMessageHandler<T, IMessage> {
    ...
    public IMessage onMessage(T message, MessageContext ctx) {
        if (ctx.side.isClient()) {
            return this.handleClientMessage(mod_JRMCore.proxy.getPlayerEntity(ctx), message, ctx);
        }
        return this.handleServerMessage(mod_JRMCore.proxy.getPlayerEntity(ctx), message, ctx);
    }
}
